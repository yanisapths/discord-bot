  ...
  await database.getServerSettings(userGuild.id, async (serverSettings) => {
    ...
    if (serverSettings.blacklist.some((element) => text.includes(element)))
      return await message.reply(
        getLocale(serverSettings.language, "mailBlacklisted")
      );
    ...
    let userCode = userCodes.get(
      message.author.id + userGuilds.get(message.author.id).id
    );
    if (userCode && userCode.code === text) {
      userTimeout.resetWaitTime();
      const roleVerified = userGuilds
        .get(message.author.id)
        .roles.cache.find(
          (role) => role.id === serverSettings.verifiedRoleName
        );
      const roleUnverified = userGuilds
        .get(message.author.id)
        .roles.cache.find(
          (role) => role.id === serverSettings.unverifiedRoleName
        );

      database.getEmailUser(
        userCode.email,
        userGuilds.get(message.author.id).id,
        async (currentUserEmail) => {
          let member = await bot.guilds.cache
            .get(currentUserEmail.guildID)
            .members.cache.get(currentUserEmail.userID);
          ...
          if (member) {
            ...
            try {
              await member.send(
                "You got unverified on " +
                  userGuilds.get(message.author.id).name +
                  " because somebody else used that email!"
              );
            } catch {}
          }
        }
      ...
      let verify_client;
      try {
        verify_client = userGuilds
          .get(message.author.id)
          .members.cache.get(message.author.id);
        await verify_client.roles.add(roleVerified);
      } catch (e) {
        message.author
          .send(getLocale(serverSettings.language, "userCantFindRole"))
          .catch(() => {});
        return;
      }
      try {
      ...
      try {
        if (serverSettings.logChannel !== "") {
          userGuilds
            .get(message.author.id)
            .channels.cache.get(serverSettings.logChannel)
            .send(
              `Authorized: <@${message.author.id}>\t →\t ${userCode.logEmail}`
            )
            .catch(() => {});
        }
      } catch {}
      await message.reply(
        getLocale(serverSettings.language, "roleAdded", roleVerified.name)
      );
      userCodes.delete(
        message.author.id + userGuilds.get(message.author.id).id
      );
      ...
      for (const domain of serverSettings.domains) {
        let regex = new RegExp(
          domain.replace(/\./g, "\\.").replace(/\*/g, ".+").concat("$")
        );
        if (regex.test(text)) {
          validEmail = true;
        }
      ...
      if (text.includes(" ") || !validEmail) {
        ...
        const infoLink = buildVerificationLink(
          "https://poc-verify.vercel.app/info",
          uniqueToken,
          queryParams
      );
        const url = new URL(infoLink);
        await message.reply(
          `Hi, before joining our server, we'd like to get to know you. Please fill some information about yourself: ${url}`
        );
      } else {
        let timeoutSeconds =
          userTimeout.timestamp + userTimeout.waitseconds * 1000 - Date.now();
        if (timeoutSeconds > 0) {
          await message.author.send(
            getLocale(
              serverSettings.language,
              "mailTimeout",
              (timeoutSeconds / 1000).toFixed(2)
            )
          );
          return;
        }
